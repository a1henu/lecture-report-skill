...
### 7. 质量评审 + 优化 —— 派评审 agent 并行审阅各章

技术校验通过后（文件完整、锚点无误），进入**教学质量评审**。目标：让报告更深入浅出、初学者友好、便于复习。

**做法**：在同一条消息里并行派发评审 subagent（`subagent_type: general-purpose`，模型继承主会话），**一章一个评审 agent**。每个评审 agent 的 prompt 用 `reference/review-subagent-prompt.md` 模板。评审 agent 会：

1. 从四个维度评估该章报告：
   - **易学性**：直觉铺垫是否充分、过渡衔接是否自然、术语首现是否有解释
   - **深度**：证明是否跳步、例题是否全覆盖、多解法是否充分
   - **深入浅出**：是否有类比/记忆锚点、是否从特例推广到一般
   - **复习友好度**：速查表是否完整、易错点是否标出

2. **直接修改文件**（用 Edit 工具）——不是只出评审报告，而是亲手改好：
   - 在定义/定理前补加直觉铺垫（`<div class="box insight">`）
   - 补全证明/例题中的跳步
   - 加入类比、记忆口诀、易错提醒
   - 完善章末速查表
   - 把教科书腔改为更自然的表述

3. 返回该章的易学性评分（1-10）和改进摘要。

**体量控制**：
- 章节数 ≤ 6 时一次性全部并行派发。
- 章节数 > 6 时分两批（前半 / 后半），避免并发过多。
- 评审 agent 的修改量控制在合理范围——重点打磨「初学者看不懂」的痛点，非事无巨细。

**兜底**：若某章评分 < 6（说明仍难学），主 agent 读取评审反馈后亲自再改一轮，或再派一次针对性修补 agent。
